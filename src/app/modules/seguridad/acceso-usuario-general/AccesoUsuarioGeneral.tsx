...
import TablaChildUsuarioGeneral from './components/TablaChildUsuarioGeneral';
...
import { useAccesoUsuarioGeneral } from './services/useAccesoUsuarioGeneral';
...
const AccesoUsuarioGeneral = () => {
  ...
  const { loadApiGetUsuariosforVentas, loadApiGetgetSearchUsuarioVenta, loadApiSetUpdatePermiso } = useAccesoUsuarioGeneral();
  ...
  const loadGetUsuariosforVentas = async () => {
    try {
      const response = await loadApiGetUsuariosforVentas();
  ...
  const loadGetgetSearchUsuarioVenta = async () => {
    ...
    try {
      setLoading(true);
      const response = await loadApiGetgetSearchUsuarioVenta(idUsuario);
      setLoading(false);
  ...
  const CustomRenderCheck = (ACCEDE: number, ID_VENTAS_ACCESO: string) => {
    ...
    const hadleChangeCheked = async (index: number, estado: boolean, idVentasAcceso: string) => {
      ...
      if (estado) {
        setLoadingX(true);
        const response = await loadApiSetUpdatePermiso("1", idUsuario, idVentasAcceso)
        ...
        setLoadingX(true);
        const response = await loadApiSetUpdatePermiso("0", idUsuario, idVentasAcceso)
        setValue("padre", !!estado)
        setLoadingX(false);
        if (response?.status && response?.message) {
          AlertSave({ title: '', message: `${response?.message}` })
...
/> : null}
  ...
  function SidebarItem(elemento: any) {
    ...
    if (SUBNIVEL == undefined || SUBNIVEL?.length <= 0) {
      ...
      arrayTemp.push(item)
      return (
        <TablaChildUsuarioGeneral idUsuario={idUsuario} lista={arrayTemp} />
      )
    }
            ...
            <Grid container columnSpacing={{ xs: 1, sm: 2, md: 2 }} >
              ...
              <Grid item xs={12} sm={6} md={4}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <h5 style={{ margin: "5px" }}>Usuario</h5>
                  <Autocomplete
                    disablePortal
                    id="combo-box-demo"
