...
import { FormEntradaDiario } from './components/FormEntradaDiario';
...
const EntradaDiario = () => {
  ...
  const [tituloFormulario, setTituloFormulario] = useState('Agregar Entrada de Diario');

  const columns = [
    { value: 'serie', label: 'Serie' },
    { value: 'socioComercial', label: 'Socio comercial' },
    { value: 'fecha', label: 'Fecha' },
    { value: 'total', label: 'Total' },
  ];

  const filters = [
    { label: 'Serie', field: 'serie', type: 'input' },
  ];

  const url = 'ctb/journal-entry';

  const RowComponent = ({ row, open, update, eliminar }) => {
    return (
      <TableRow>
        <TableCell>{row.serie}</TableCell>
        <TableCell>{row.socioComercial}</TableCell>
  ...
  const openModal = async (open, id) => {
    setTituloFormulario('Agregar Entrada de Diario')
    reset({ articulos: [] });
    if (id) {
      setTituloFormulario('Editar Entrada de Diario')
