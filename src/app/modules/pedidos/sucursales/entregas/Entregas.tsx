...
import { useEntregas } from './services/useEntregas';
...
import { ModalAbrirEntregas } from './components/ModalAbrirEntrega';
import { ModalCerrarEntregas } from './components/ModalCerrarEntregas';
import { SiMicrosoftexcel } from "react-icons/si";
import { ImFilePdf } from "react-icons/im";
import { AlertToastError, AlertToastWarning } from '../../../../common/alerts/alertsToast';
import PaletaEntrega from './components/PaletaEntrega';
...
const Entregas = () => {
  ...
  const { loadApiRestriccion, loadApiEntregaTurnoSucursal, loadApiEntregaSucursal, loadApiGuardarEntregaSucursal, loadApiEnviarEntregaSucursal } = useEntregas()
  ...
  const [entregada, setEntregada] = useState<any>({})
  const [devuelta, setDevuelta] = useState<any>({})
  ...
  const handleChangeDate = async (fecha: Date) => {
    ...
    loadDataEntregaParmaters(auxFormatFecha)
  ...
  //api entregas lista turno segun fecha
  const loadDataEntregaTurnoFecha = async () => {
    try {
      ...
      const response = await loadApiEntregaTurnoSucursal(idSucursal, fechaActual)
      setLoading(false)
      console.log("lista de entregas turnos ", response)
      setLista(response)
  ...
  //api entregas lista 
  const loadDataEntregaSucursal = async () => {
    try {
      ...
      const response = await loadApiEntregaSucursal(idSucursal, fechaActual, NombrePerfilTemp)
      setLoading(false)
      console.log("lista de entrega central ", response)
      ...
      getValueByKeyInObjectTagReplaceCeros(response?.aceptada,response?.entregada,"_entregada")
      getValueByKeyInObjectTag(response?.aceptada, "_aceptada")
      setAceptada(response?.aceptada)
      //getValueByKeyInObjectTag(response?.entregada, "_entregada")
      setEntregada(response?.entregada)
      getValueByKeyInObjectTag(response?.devuelta, "_devuelta")
      setDevuelta(response?.devuelta)
      ...
      setEntrega(response.recepcion)
      handleSubmit(()=> {})();

      if (response?.cabecera && response?.cabecera[0]?.ESTADO) {
        if (response?.cabecera[0]?.ESTADO == 13) {
          console.log("entre aqui ", controlDisable)
          localStorage.setItem('estaDesabilitado_entregas', JSON.stringify(false));
          return;
        }
        if (response?.cabecera[0]?.ESTADO == 14) {
          //hablitado o inabilitado
          localStorage.setItem('estaDesabilitado_entregas', JSON.stringify(true));
        }
        if (response?.cabecera[0]?.ESTADO >= 15) {
          localStorage.setItem('estaDesabilitado_entregas', JSON.stringify(true));
          return;
        }
      }
  ...
  const loadDataEntregaParmaters = async (suc_fecha: any) => {
    ...
    try {
      ...
      const response = await loadApiEntregaTurnoSucursal(idSucursal, suc_fecha)
      setLoading(false)
      console.log("lista de entregas turnos2 ", response)
      setLista(response)
  ...
  //guardar los datos de entregas
  //guardar los dataForm de entregas
  const onSubmitEntrega = async (dataForm: any) => {
      //esta vacio-> true
      ...
      if (isObjEmpty(dataForm) || isObjEmpty(registro) || !idSucursal
        || !fechaActual || isObjEmpty(observacion) || isObjEmpty(entregada) 
        || isObjEmpty(devuelta)
      ) {
      ...
      for (var key in registro) {
        ...
        // if (dataForm[`${key}_entregada`] != entregada[key] ||
        //   dataForm[`${key}_devuelta`] != devuelta[key] ||
        //   dataForm[`${key}_observacion`] != observacion[key]
        // ) {
          //console.log("el valor campo por orginal(izq) cambiado(der) ", key, devuelta[key], dataForm[`${key}_devuelta`])
      ...
      console.log("res send guardar ", resObj)
        ...
        if (isMultiploDeNumberCustom(resObj, dataForm, "_devuelta",adecuacionInput)) {
          console.log("son multiplos de 0.25")
          try {
            setLoading(true)
            const response = await loadApiGuardarEntregaSucursal(
              idSucursal,
              fechaActual,
              resObj
            )
  ...
  const onSendEntrega = async (dataForm: any) => {
    //esta vacio-> true
      //si no esta vacio-> false
      console.log("respues ", isObjEmpty(dataForm), fechaActual)
      //paso 1 verificar si el objecto es mayor a cero
      ...
      for (var key in registro) {
       ...
       const dataSend = {
         ...
         "fecha": fechaActual,  // 2023-05-10
         "tipo": NombrePerfilTemp,
         "entrega": resObj
     }
      console.log("res send enviar ", resObj)
      console.log("res send enviar todo ", dataSend)
      console.log("perfil seleccionado: ",NombrePerfilTemp)
  
        if (isMultiploDeNumberCustom(resObj, dataForm, "_devuelta",adecuacionInput)) {
          try {
            setLoading(true)
            const response = await loadApiEnviarEntregaSucursal(
              idSucursal,
              fechaActual,
              NombrePerfilTemp,
              resObj
            )
            console.log("res enviar entregas ", response)
            setLoading(false)
            if (response?.status) {
              await loadDataEntregaSucursal()
              AlertSave({ title: "", message: response.message });
...
const handleChange1 =
        ...
        for (let z = 0; z < productosTemp?.length; z++) {
          if (registro[`${productosTemp[z].ID_SUB_CATEGORIA_2}`] != ".00" && registro[`${productosTemp[z].ID_SUB_CATEGORIA_2}`] != undefined) {
            var objProd = {
              ...
              cant_entregada: entregada[`${productosTemp[z].ID_SUB_CATEGORIA_2}`] ,
              cant_devuelta: (devuelta[`${productosTemp[z].ID_SUB_CATEGORIA_2}`] == '.00' ? "0" : devuelta[`${productosTemp[z].ID_SUB_CATEGORIA_2}`]) ,
  ...
  const columns = [
    ...
    { name: "Cantidad Entregada", nameRow: "cant_entregada" },
    { name: "Cantidad Devuelta", nameRow: "cant_devuelta" },
  ...
  printCustomPDF(arrayGetOnlyProduct, columns, titleHeader,"entregas-ventas")
...
const generateExcel = () => {
  ...
  const titleHeader = (`REPORTE DE CANTIDADES PREPARADAS - ${NombreSucursalShow}`).toUpperCase()
  setLoading(true)
  exportToCustomCSV(arrayGetOnlyProduct, columns, titleHeader, "entregas-ventas")
  setLoading(false)
}
  ...
  return (
    <>
            ...
            <Grid container columnSpacing={{ xs: 1, sm: 2, md: 2 }} >
              ...
              <Grid item xs={6} sm={6} md={2} container
                ...
                <ColorButton sx={{ backgroundColor: '#43A047', marginTop: '29px' }}
                  variant="contained"
                  onClick={loadDataEntregaSucursal}
                >
                  Buscar</ColorButton>
              </Grid>
            ...
            <div>
              {cabecera?.map((row: any, index: any) => {
                ...
                /*if (row.ESTADO == 13) {
                  return (
                    <>
                      <Button sx={{ fontSize: '1.6em', color: 'white' }} onClick={handleSubmit(onSubmitEntrega)}><IoSaveOutline /></Button>
                      <Button sx={{ fontSize: '1.6em', color: 'white' }} onClick={handleSubmit(onSendEntrega)}><IoMdPaperPlane /></Button>
                    </>
                  )
                }
                if (row.ESTADO == 14) {
              ...
              <Button onClick={generateExcel} sx={{ fontSize: '1.5em', color: 'white' }}><SiMicrosoftexcel /></Button>
              <Button onClick={formatDataforPdf} sx={{ fontSize: '1.5em', color: 'white' }}><ImFilePdf /></Button>
            </div>
      ...
      {true ?
        <>
          {cabecera[0]?.ESTADO >= 13 && existencia?.map((row: any, index: any) => {
            return (
      ...
      {cabecera[0]?.ESTADO < 13 || cabecera?.length <= 0 ?
        <>
          {messageEmpty({ message: 'NO SE HA ENVIADO RECEPCION DEL DIA DE LA FECHA SELECCIONADA' })}
        </>
      : null}
