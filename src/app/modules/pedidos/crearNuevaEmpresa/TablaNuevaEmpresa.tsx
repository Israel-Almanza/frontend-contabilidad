...
//const rows = 'calories';
const DEFAULT_ORDER_BY = 'ID_AREA_PRODUCCION';
    ...
    <TableHead style={{
      ...
      <TableRow sx={{
        ...
        >
          {/* Nombre Completo*/}
          <TableSortLabel
            active={orderBy === "ID_AREA_PRODUCCION"}
            direction={orderBy === "ID_AREA_PRODUCCION" ? order : 'asc'}
            onClick={createSortHandler("ID_AREA_PRODUCCION")}
          >
            ID_EMPRESA
            {orderBy === "ID_AREA_PRODUCCION" ? (
        ...
        <TableCell sx={{ ...tableStyling, width: '18%', minWidth: '60px', borderLeft: "1px solid white" }} align="center">
          NOMBRE EMPRESA
          <TableSortLabel
        ...
        <TableCell sx={{ ...tableStyling, width: '11%', borderLeft: "1px solid white" }} align="center">
          DESCUENTO UNITARIO
        </TableCell>
        <TableCell sx={{ ...tableStyling, width: '11%', borderLeft: "1px solid white" }} align="center">
          DESCUENTO ICE
        ...
        <TableCell sx={{ ...tableStyling, width: '7%', borderLeft: "1px solid white" }} align="center">
          OPCIONES
        </TableCell>
...
export default function TablaNewEmpresa(props: any) {

  //console.log("rec prodiedad ", props.lista)
  const { loadApiCambioEstado } = useNewEmpresa()
  const { tableData, deleteByIndex, loadObtenerListaEmpresa, handleSubmit, control, register, getValues, setValue }: any = props;
  ...
  const onClickCambiarEstado = async (ID_AREA_PRODUCCION: string, estado: string, textDinamic: string) => {

    console.log("pasando parametros ", ID_AREA_PRODUCCION, estado, textDinamic)
    try {
                ...
                return (
                  <TableRow key={index}>
                    ...
                    >
                      {row.ESTADO == 1 ?
                        <>
                          <ColorButtonGreen variant="contained" sx={{ marginTop: '0px',fontSize:'11px', padding: '2px' }}
                            title='Desactivar Empresa'
                            onClick={() => onClickCambiarEstado(row.ID_AREA_PRODUCCION,"0",'Se Desactivara la Empresa')}
                          >ACTIVO</ColorButtonGreen>
                        </>
                        :
                        <>
                          <ColorButton variant="contained" sx={{ marginTop: '0px',fontSize:'11px', padding: '2px' }}
                            title='Activar Empresa'
                            onClick={() => onClickCambiarEstado(row.ID_AREA_PRODUCCION,"1",'Se Activara la Empresa')}
                          >INACTIVO</ColorButton>
                        </>}
                    ...
                    <TableCell
                      ...
                      align="center"
                    >
                      <Button sx={{ fontSize: 25,color: 'orange' }} 
                      onClick={() => pasarParametrosEditar(row)}><TiEdit/></Button>
                    </TableCell>
                  </TableRow>
                );
              })}
