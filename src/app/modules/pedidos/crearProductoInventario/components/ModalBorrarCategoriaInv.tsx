...
export const ModalBorrarCategoriaInventario = (props:any) => {
    const { loadApiEliminarCategoriaInventario} = useProductosInventario()
    const {openModalBorrarCategoriaINV,handleOpenModalBorrarCategoriaINV ,handleCloseModalBorrarCategoriaINV, loadObtenerCategoria, idPrimeraCat, estado} = props;
//loading
...
const loadBorrarCategoria = async () => {
    ...
    try {
        setLoading(true)
      const response = await loadApiEliminarCategoriaInventario(
        idPrimeraCat,
        "0"
      );
      console.log("res eliminar categoria ", response)
      handleCloseModalBorrarCategoriaINV()
            setLoading(false)
            if (response?.status) {
    ...
    return (
        <Modal
            open={openModalBorrarCategoriaINV}
        ...
        >
            <Box sx={styleModal}>
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
                    ...
                    &nbsp; &nbsp;
                    <Button onClick={handleCloseModalBorrarCategoriaINV} sx={{ backgroundColor: '#DC3741' }} variant="contained" >No </Button>
