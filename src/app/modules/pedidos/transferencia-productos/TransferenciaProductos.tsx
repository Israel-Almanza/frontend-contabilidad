...
import TablaTransferenciaProductos from './TablaTransferenciaProductos';
...
const TransferenciaProductos = () => {
  ...
  const { loadApiSucursalesUsaurio, loadApiTransferProduct } = useTransferenciaProductos()
  ...
  const requestSearch = (searchedVal: string) => {
    ...
    const keys = ["FECHA", "AREA_PRODUCCION"]
    const filteredRows = originalRows.filter((row: any) => {
  ...
  const loadTransferProduct = async () => {
    try {
      ...
      setLoading(true)
      const response = await loadApiTransferProduct(Number(idSucursal), fechaInicio, fechaFin)
      setLoading(false)
  ...
  const printPDF = () => {
    const titleHeader = `Transferencia de Productos ${nombreSucursalTabla} `
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      var temp = [
        // nombreSucursalTabla,
        formatDateString(rows[i].FECHA),
        rows[i].ENVIO == 1 ? "Enviado" : "Recibido",
        rows[i].AREA_PRODUCCION,
  ...
  function customersData() {
    ...
    for (let i = 0; i < rows.length; i++) {
      custs[i] = {
        ...
        "Fecha": formatDateString(rows[i].FECHA),
        "Operación": rows[i].ENVIO == 1 ? "Enviado" : "Recibido",
        "Sucursal": rows[i].AREA_PRODUCCION
  ...
  const wscols = [
    ...
    { wch: Math.max(...rows.map((customer: any) => customer?.FECHA?.length)) },
    { wch: Math.max(...rows.map((customer: any) => 5)) + 10 },
    { wch: Math.max(...rows.map((customer: any) => customer?.AREA_PRODUCCION?.length)) },
