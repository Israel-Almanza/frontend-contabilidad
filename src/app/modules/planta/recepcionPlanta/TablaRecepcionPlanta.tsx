...
export default function TablaRecepcionPlanta(props: any) {
  const {loadApiListarDetalleRecepcion} = useRecepcionPlanta()
  //console.log("rec prodiedad ", props.lista)
  const { tableData, 
    deleteByIndex, updateTextTablaRecepcion, control, getValues, handleSubmit, setValue, reset,
    handleOBoton, handleCBoton }: any = props;
  ...
  const pasarParametrosDetalle = async (row: any) => {
    
    const { CODIGO, FECHA_REGISTRO } = row;
    console.log("row ", row)
    //setCodigo(CODIGO)
    ...
    try {
      setLoading(true)
      const response = await loadApiListarDetalleRecepcion(
        CODIGO,
        FECHA_REGISTRO
      );
      setLoading(false)
      console.log("lista detalle ", response)
      handleOBoton()
      if (response?.status && response?.respuesta) {
        //setListaDetalle(response.respuesta)
        updateTextTablaRecepcion(response?.respuesta)
 ...
 // console.log("lista detalle t",ListaDetalle)
