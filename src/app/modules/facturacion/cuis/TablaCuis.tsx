...
import { ModalActivarCuis } from "./components/ModalActivar";
import { ModalDesactivarCuis } from "./components/ModalDesactivar";
...
import { ModalMostrarCuis } from "./components/ModalCuis";
  ...
  return (
    <TableHead style={{
      ...
      <TableRow sx={{
        ...
        <TableCell sx={{ ...tableStyling, width: '15%', minWidth: '60px', borderLeft: "1px solid white", wordBreak: 'normal' }} align="left">
          Codigo Sistema
          <TableSortLabel
            active={orderBy === "ci"}
        ...
        <TableCell sx={{ ...tableStyling, width: '4%', borderLeft: "1px solid white", wordBreak: 'normal' }} align="left">
          Cod. Mod.
          <TableSortLabel
            active={orderBy === "nombre"}
            direction={orderBy === "nombre" ? order : 'asc'}
        ...
        <TableCell sx={{ ...tableStyling, width: '9%', borderLeft: "1px solid white", wordBreak: 'normal' }} align="left">
          Cod. Suc.
          <TableSortLabel
            active={orderBy === "nombre"}
            direction={orderBy === "nombre" ? order : 'asc'}
        ...
        <TableCell sx={{ ...tableStyling, width: '6%', borderLeft: "1px solid white", wordBreak: 'normal' }} align="left">
          Cod. 
          <TableSortLabel
            active={orderBy === "nombre"}
            direction={orderBy === "nombre" ? order : 'asc'}
        ...
        <TableCell sx={{ ...tableStyling, width: '2%', borderLeft: "1px solid white", wordBreak: 'normal' }} align="left">
          Opciones
...
export default function TablaCuis(props: any) {
  const {loadApiVerCuis} = useCuis()
  const { tableData, deleteByIndex, loadObtenerListaCuis }: any = props;
  const { formState, handleSubmit, control, register, getValues, setValue, unregister } = useForm();
//<---modal
const [openModalCuis, setOpenModalCuis] = useState(false);

const handleOpenModalCuis = () => setOpenModalCuis(true);
const handleCloseModalCuis = () => setOpenModalCuis(false);
//modal--->
  ...
  const [Id_Cuis, setId_Cuis] = useState('');
 
   const pasarParametrosCuis = async (row: any) => {
    const { TOKEN_API, ID_VENTAS_F01_CUIS } = row;
    console.log("row ", row)
    //setToken(TOKEN_API)
    handleOpenModalCuis()
    try {
      setLoading(true)
      const response = await loadApiVerCuis(ID_VENTAS_F01_CUIS)
      setLoading(false)
      console.log("lista cuis ", response)
      
      //if(response?.status && response?.response){
        setCuis(response.response)
      //}
  ...
  const pasarParametrosA = (row: any) => {
    const { ID_VENTAS_F01_CUIS } = row;
    console.log("row ", row)
    setId_Cuis(ID_VENTAS_F01_CUIS)
  ...
  return (
    <div>
                ...
                return (
                  <TableRow key={index}>
                    ...
                    >
                       {row.ESTADO == 1 ?
                        <>
                          <ColorButtonGreen variant="contained" sx={{ marginTop: '0px', fontSize:'10px' }} 
                          onClick={() => pasarParametrosDes(row)}
                          >HABILITADO</ColorButtonGreen>
                        </>
                        :
                        <>
                          <ColorButton variant="contained" sx={{ marginTop: '0px', fontSize:'10px' }}
                    ...
                    <TableCell
                      ...
                      align="left"
                    >
                      <Button sx={{ fontSize: 20,color: '#42A5F5' }} onClick={()=>pasarParametrosCuis(row)}><ImEye/></Button>
      ...
      <ModalMostrarCuis
        openModalCuis={openModalCuis}
        handleOpenModalCuis={handleOpenModalCuis}
        handleCloseModalCuis={handleCloseModalCuis}
        Cuis={Cuis}
      />
