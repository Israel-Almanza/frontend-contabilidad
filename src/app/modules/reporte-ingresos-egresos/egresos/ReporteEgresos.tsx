...
import { useReporteEgresosServices } from './services/useReporteEgresosServices';
...
const ReporteEgresos = () => {
    ...
    const { loadApiListaTipoReporte, loadApiReporteFechas } = useReporteEgresosServices();
    ...
    useEffect(() => {
        ...
        console.log("valor del item  reportes", item, nombre, sucursal);
    ...
    const loadListaReporteFecha = async () => {
        try {
            ...
            setLoading(true)
            const response = await loadApiReporteFechas(
                fechaInicio,
                fechaFin,
                location.pathname
            )
                            ...
                            {/* <Grid item xs={12} sm={3} md={3}>
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <Autocomplete
                                        ...
                                        onChange={(event, item) => {
                                            handleSeleccioneTipoReporte(item)
                                            //onChange(item)
                                        ...
                                        renderOption={(props, option, index) => {
                                            const key = `listItem-${index}-${option.ID_PROCEDIMIENTOS_REPORTES}`;
                                            return (
                                                <li {...props} key={key}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                                        <DesktopDatePicker sx={{ width: '100%' }}
                                            defaultValue={dayjs() as unknown as Date}
                                            format='DD/MM/YYYY'
                                            onChange={(newValue: any) => { handleChangeDateFin(newValue) }}
                                            slotProps={{ textField: { size: 'small' } }}
